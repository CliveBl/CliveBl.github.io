...
export function editableGetDocTypes() {
  ...
  return Array.from(document.querySelectorAll("#expandableAreaUploadFiles #accordionContainer"))
    ...
    .filter((type): type is string => Boolean(type)); // Remove any null/undefined values
}
...
export async function displayFileInfoInExpandableArea(allFilesData: any, backupAllFilesData: any, isNewlyUploadedFile = false) {
  ...
  function displayFileInfoHeader(expandableArea: HTMLDivElement, data: any) {
    ...
    const headerCaptions = [
      ...
      { text: "סוג מסמך", width: "190px" },
      { text: "שם האירגון", width: "200px" },
      { text: "שם", width: "200px" },
  ...
  function displayFileInfoLine(accordianBody: HTMLDivElement, headerFieldsContainer: HTMLDivElement, fileData: any) {
    ...
    function createHeaderInput(value: any, fieldName: string, isEditable = true) {
      ...
      headerFieldlabel.textContent = getFriendlyName(fieldName);
